...
const ProductivityProgress = ({ score }: { score: number; }) => {
  ...
  const styles = {
    ...
    container: {
      ...
      border: `1px solid ${isComplete ? colors.BLUE : colors.LIGHT_BROWN}`,
      borderRadius: 4,
      marginBottom: 24,
      height: 8
    },
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 8, marginTop: 8 }}>
            ...
            <div style={{ fontSize: 10, color: '#B88C5A' }}>average this week</div>
          </div>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: '#B88C5A' }}>average this month</div>
          </div>
        </div>
...
const SectionSuggestions = ({ suggestions }: SectionSuggestionProps) => {
  ...
  return (
    <div style={{ marginTop: 32, marginBottom: 32 }}>
      ...
      <ul style={{ padding: '0 8px', listStyle: 'none', fontSize: 14, marginBottom: 16 }}>
        {
          tasks.map(task => {
            const { id, name } = task;
...
> {
  ...
  render() {
    ...
    const highImpactTasksWellness = wellnessScoresByTask.slice(0, 3)
      .reverse()
      .map(({ task, data }) => {
        const { average } = data;
      ...
      <div className='activity-details-container'>
        ...
        */}
        ...
        <AnalyticsSectionPreview
          ...
          topIssues={
            wellBeingIssues.slice(0, 3).map(({ question, average }) => {
        ...
        <AnalyticsSectionPreview
          ...
          topIssues={
            anxietyQuestionStats.slice(0, 3).map(({ question, average }) => {
              const score = (average / 4);
        ...
        <AnalyticsSectionPreview
          ...
          topIssues={
            depressionQuestionStats.slice(0, 3).map(({ question, average }) => {
              const score = (average / 4);
