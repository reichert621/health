...
const MonthlyAveragesTable = ({ stats }: { stats: MonthlyAverageStats }) => {
  ...
  return (
    <table className="dashboard-list-table" style={styles.container}>
      ...
      <tbody>
        <MonthlyAverageRow label="Productivity" months={months} averages={p} />

        <MonthlyAverageRow label="Depression" months={months} averages={d} />

        <MonthlyAverageRow label="Anxiety" months={months} averages={a} />

        <MonthlyAverageRow label="Well-being" months={months} averages={w} />
      </tbody>
    </table>
  );
};
